...
Select HVG
```{r}
decomp <- modelGeneVar(sce) %>% as.data.table(keep.rownames = T) %>% setnames("rn","gene") %>% setorder(FDR,na.last=T)
...
Plot PCA
```{r}
plotPCA(sce_filt, colour_by="sample", ncomponents = c(2,1))
...
plotPCA(sce_tmp, colour_by="sample", ncomponents = c(1,2))
```
...
interesting.cells.jasmin <- fread("/Users/argelagr/data/DUX4_hESCs_multiome/results/rna/DUX4-OE_seurat-clusters.txt") %>%
  .[V2==6,V1]
